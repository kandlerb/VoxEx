...
- **Still open (carried from Phase B, unaffected by Phase C):** opening the inventory mid-channel does not end an active laser/freeze channel (see Phase B's As-built above for the full trace) — needs a human decision on whether/how to fix.
...
2. **Inventory-open mid-channel gap** (pre-existing, surfaced by Phase B): decide whether to amend this CCR to accept it as a known limitation, or authorize a small follow-up fix to `resetTransientInput`/`openInventory`/`exitGameplay`.
