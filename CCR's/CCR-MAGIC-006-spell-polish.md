...
> **Status: PHASES A-C IMPLEMENTED** (2026-07-08, builds `2026-07-08.1`/`.2`/`.3`, all on `main`) — DRAFT → AUDITED → **IMPLEMENTED (A-C)** → Phases D/E remain deferred (see the Run Summary in As-built below); move to `Finished/` once D/E land or are formally dropped.
...
Phases A, B, and C below are complete (per-phase deviations, final `NUM_TILES = 40`, and test counts are recorded in each phase's own section, with a consolidated "Run summary" after Phase C's). **The r=4/6/8 carve measurements and the Stage-1 vs Stage-2 decision are still pending** — that gate belongs to Phase D, which is out of scope for this run; see the Run summary below for what's still open.
...
- **Discovered gap, NOT fixed (surfaced to human, cycle 2):** investigating the remaining two named behaviors ("inventory-open mid-channel ends it" and "channelActive never true with magicMode false") surfaced a genuine, pre-existing bug the reviewer independently confirmed by tracing the same call chain: `uiManager.openInventory()` → `exitGameplay("inventory")` → (desktop) `controls.unlock()` → `onGameplayFocusLost()` → `resetTransientInput()`. That last function resets `leftMouseHeld`/`rightMouseHeld`/`breakingBlock` etc. but **never calls `stopMining()` or `endChannel()`**. The CCR's own B1 rationale for routing everything through `stopMining()` ("verified: mouse-up, mode toggle, touch blur/pointercancel, pause, inventory-open all flow through it") is factually wrong for the inventory-open path — this predates Phase B (nothing in this diff touches that chain) but is now a real, visible symptom for the first time: **opening the inventory mid-laser-channel leaves the beam mesh + persistent light frozen in-scene** (frozen, not ticking, since `isGameplayActive()` gates `tickChannel`) until a later mouseup eventually fires `endChannel()`. Not fixed here — do-not list item 5 restricts `endChannel()` to `stopMining()`/`onMouseUp` only, and the actual fix site (`resetTransientInput`/`openInventory`/`exitGameplay`) is outside this CCR's B1 edit surface. **Needs a human decision**: either amend this CCR to accept the gap as known, or authorize a small follow-up (likely: call `stopMining()` — not `endChannel()` directly, to stay within the do-not list's letter — from `resetTransientInput()` or `onGameplayFocusLost()`). "`channelActive` never true with `magicMode` false" was left uncovered too, but is a design characteristic (not a regression): `beginChannel` has never checked `magicMode` itself, same caller-gated pattern `castSpell` already used pre-Phase-B.
  ...
  **Fixed 2026-07-11:** did exactly the human-decision suggestion above — `resetTransientInput()` (grep `function resetTransientInput`) now calls `stopMining()` (not `endChannel()` directly, staying within the do-not list's letter). Since `onGameplayFocusLost()` already called `resetTransientInput()`, and `exitGameplay()`'s inventory-open path already reached `onGameplayFocusLost()`, this one added call closes the whole chain: `uiManager.openInventory()` → `exitGameplay("inventory")` → `onGameplayFocusLost()` → `resetTransientInput()` → `stopMining()` → `endChannel()`. `stopMining()`'s own guards (`if (breakingBlock)`, `endChannel`'s `if (!channelActive) return`) make the added call idempotent against the pre-existing `onMouseUp`/mode-toggle release funnels, so no double-fire risk. See CLAUDE.md's Magic System section for the current one-line summary.
...
### Phase B follow-up fixes (build 2026-07-08.4, from first real play-test)

Two laser-channel bugs Kandler hit in-game, fixed directly (small surgical diff, prototyped and suite-verified on a git-archive copy because the sandbox mount was serving truncated `voxEx.html` reads):

- **Sweep carve deadlock (spec bug in B2/F5 as written):** the CCR's F5 clamp re-grounds `channelDepth` but says nothing about `_lastCarvedDepth`. After a deep dig, sweeping onto nearer terrain clamps `channelDepth` below the carve cursor, so the F17 whole-voxel check (`floor(channelDepth) > floor(_lastCarvedDepth)`) can never fire again — the laser permanently stops carving for the rest of the channel. Fix: new pure `nextLaserCarveCursor(prevDepth, newDepth, cursor)` — when a tick's clamp actually shortens the beam, the cursor drops to `newDepth - 1`; unchanged during forward digging and at a pinned bedrock face (no per-frame churn). Seam-exported; +5 suite tests including the exact deadlock sequence. **This was a flaw in the audited spec itself, not an implementation error** — the audit's F5 "clamp the frontier" fix was necessary but not sufficient.
...
- **C1/C2 implemented as specified.** `castFireball` raycasts `SPELL_TARGET_RANGE` and targets the AIR side of the hit face (F6) instead of the free-flight gravity arc; a deterministic path-mode projectile (`pathMode`/`pathFrom`/`pathTo`/`pathT`/`pathDur`/`arcHeight`) is guaranteed to arrive exactly at the aimed target, while the existing per-frame solid/mob early-detonation checks are preserved. `onFireballImpact` chars a power-scaled `charRadius` sphere of burnable blocks instantly (skipping the burn-timer/cling delay) before `igniteFireballBurst` runs (F14).
- **Deviation (correct, not a bug):** `SPELL_FIRE.params.gravity` was removed from the config entry — `castFireball`'s new path-mode delivery never reads it (nothing else in the shipped spell set does either), so it was genuinely dead once C1 landed. Not explicitly called out in the CCR's literal C1 text, but consistent with how Phase A already removed `params.knockback`/`FIREBALL_TUNING.igniteMax` when they became dead.
- **Extraction for testability (judgment call, same precedent as Phase B's `nextLaserChannelDepth`):** `computeFireballTarget` (F6 target selection) and `pathProjectilePosition` (the guaranteed-arrival parabola) were pulled out of `castFireball`/`updateProjectiles` into small pure functions — `spawnProjectile` itself returns `null` without a live `scene` (test mode), so the surrounding spell-cast/projectile-pool machinery is unreachable via the `?test=1` seam; extracting the pure math is what makes C1's Verify-line intent ("impact lands exactly at the raycast prediction") actually testable.
- **C3 implemented as specified**, including the generic `drawCrackOverlay` texture stamp (copied over each base tile's own generator, not independently authored), `CRACKED_STONE`/`CRACKED_DIRT`/`CRACKED_PLANKS` (tiles 37-39, `NUM_TILES` 37→40 in both copies), the `CRACKED_VARIANT` lookup built in the existing `burnsTo`/`burnTime` compile loop (F15: no variant re-cracks), and the generic scar rule wired at all three named sites (explosion rim, fireball impact patch, laser bore walls).
- **Review-caught defect, fixed before commit (cycle 1):** the first draft of the laser bore-wall scarring (`scarTubeWalls`) mirrored `carveTubeEdit`'s own per-DDA-step stamp idiom (`stampBoreCrossSection`) — a per-step 3D distance-from-center shell check. That idiom is safe for CARVING (idempotent; overlapping stamps from consecutive steps are harmless) but wrong for SCARRING: since consecutive DDA steps are only ~1 unit apart while the outer shell radius is `boreRadius+1` (≥2), a *later* step's local shell check could misclassify a point *behind or ahead along the tube's own axis* — including the segment's own on-axis starting voxel — as "just outside the bore," when it's actually part of the (already-carved) interior. A fresh independent reviewer caught this by hand-tracing the exact test inputs, and separately found that the two new C3 browser tests stubbed `Math.random` on the *parent test page* instead of the hidden game iframe's own `window` (`VoxEx._doc.defaultView.Math`) — a different JS realm the scarring functions' bare `Math.random()` calls never touch, which made one test flaky/non-deterministic and silently missed the geometry bug in the other. Both fixed: `scarTubeWalls` now scans the segment's bounding box once and classifies each candidate voxel by its TRUE perpendicular distance to the *clamped line segment* `[origin, origin + dir*range]` (project onto the axis, clamp to `[0, range]`, measure from that point) — the same "distance to a capsule" technique, structurally modeled on `convertConeEdit`'s bounding-box-scan style rather than `carveTubeEdit`'s per-step-stamp style. The now-obsolete per-step `scarBoreShell` helper was deleted entirely. Reviewer re-reviewed the fix (cycle 2) and independently re-derived the corrected geometry by hand before APPROVing.
- No terrain changes this phase; `NUM_TILES` 37 → 40 in both copies (parity-check P9). No `SETTINGS`/`DEFAULTS` entries added.
- **Test count: 357 → 369.**
- Gates: `syntax-check` GREEN, `parity-check` GREEN, full browser suite 369/369 GREEN.
- `tools/voxex-texture-tests.html` updated (its own independently-numbered mini-atlas: `NUM_TILES` 22 → 25, 3 new tile draws + a "crack overlay darkens the tile" sanity check) but **not run headlessly** — unlike `voxex-tests.html`, this file has no `run-browser-tests.mjs`-style automated runner; it needs a human to open it in a browser and eyeball the pass/fail grid.
- **NEEDS VERIFICATION (in-game, real hardware — this environment can't render):** long-range fireballs land exactly on the crosshair, including clipping/early-detonating correctly under a ceiling; power-5 fireball into a tree instantly chars a core + spreads live fire around it, power-1 chars nothing but still ignites; explosion craters / laser bore walls / fireball char patches show cracked-block rims wherever the underlying material is STONE/DIRT/WOOD.
- **Still open (carried from Phase A, unaffected by Phase C):** `EXPLOSION_POWER_CAP` stays at 3 — the §Approach carve-measurement gate (`window.debugCarveSphere` at r=4/6/8 on real hardware) was never run in this environment. Required before Phase D or before raising explosion above power 3.
- **Still open (carried from Phase B, unaffected by Phase C):** opening the inventory mid-channel does not end an active laser/freeze channel (see Phase B's As-built above for the full trace) — needs a human decision on whether/how to fix. **Fixed 2026-07-11** — see the note appended to Phase B's As-built entry above.

## Run summary (Phases A-C complete, D-E deferred)

All three approved phases (A, B, C — see decisions log #8) are implemented, reviewed by a fresh independent reviewer each (5 total review cycles across the three phases: 2 for A, 2 for B, 2 for C — each phase's second cycle re-reviewed a fix rather than starting over), gated green, and committed to `main` as three separate commits (one per phase, per decisions log #9). Final state: `NUM_TILES = 40`, 23 block types, browser suite at 369/369, builds `2026-07-08.1`/`.2`/`.3`.

Two items remain genuinely open and need a human decision before being considered resolved (neither blocks Phases A-C from being considered done, since both are either explicitly deferred by this CCR's own scope (the carve-measurement gate, needed only for Phase D) or predate this CCR entirely and were only surfaced, not caused, by it (the inventory-open channel gap)):
1. **Carve-measurement gate** (blocks Phase D specifically): run `window.debugCarveSphere` at r=4/6/8 on real hardware, record the result here, then decide Stage-1-vs-Stage-2 `bulkEdit` per the CCR's §Approach.
2. **Inventory-open mid-channel gap** (pre-existing, surfaced by Phase B): decide whether to amend this CCR to accept it as a known limitation, or authorize a small follow-up fix to `resetTransientInput`/`openInventory`/`exitGameplay`. **Fixed 2026-07-11** (`resetTransientInput()` now calls `stopMining()`) — see the note on Phase B's As-built entry above.

This CCR is NOT moved to `CCR's/Finished/` — Phases D and E remain deliberately out of scope for this run (per the Orchestration plan's approved-scope note) and are separate future runs; the file stays in the active `CCR's/` directory until D/E land or are formally dropped (see the CLAUDE.md Documentation Index entry for this file).
