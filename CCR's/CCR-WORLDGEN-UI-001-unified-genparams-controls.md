...
**DEFECT found in the wild (hotfixed build 2026-07-11.4):** change #13's spawn-chunk derivation shipped as `Math.floor(spawnBX / CHUNK_SIZE)` — but `CHUNK_SIZE` is a WORKER-TEMPLATE-ONLY binding (no module-scope declaration; the seam's SEAM-RECONCILE comment documents this), so EVERY world creation threw `ReferenceError: CHUNK_SIZE is not defined` in builds 2026-07-11.1–.3. The bug originated in this CCR's own #13 "After" sketch (spec bug, faithfully implemented). Nothing in the verification ladder could catch it: syntax-check only parses (unresolved identifiers are runtime errors), the browser suite can't reach `initGameEngine` (needs a live scene), and CLAUDE.md's Key Constants row listing `CHUNK_SIZE / WORLD_DIMS.chunkSize` side by side made the bare name look module-scoped. Fixed to `WORLD_DIMS.chunkSize`; Key Constants row annotated with the trap. Lesson: in-game gates exist precisely because `initGameEngine` and other scene-dependent paths are invisible to every headless check — run the create-world click before calling a build done.
