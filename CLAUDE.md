...
│   │                             #   caves preview removed — CCR-WORLDGEN-UI-001). Per-pass
│   │                             #   views (CCR-WORLDGEN-UI-002): grouped PASS_REGISTRY pass
│   │                             #   selector (19 passes), per-pass tunable-panel filtering
│   │                             #   (+ "Show all params"), diff + A/B cached-grid toggle,
│   │                             #   19-thumb commit-time filmstrip — all via ?test=1 seam,
│   │                             #   no hand-copied terrain math. CCR-WORLDGEN-CONTINENTAL-
│   │                             #   OCEANS-001: the continentalness/oceanFactor/preRiver/carved
│   │                             #   passes now surface the 'Continents & Oceans' tunable section
...
**C-authored oceans (CCR-WORLDGEN-CONTINENTAL-OCEANS-001, shipped default at build `2026-07-15.2`, `TERRAIN_GEN_VERSION` 43):** a third flag — `WORLD_CONFIG.continentalOceans: true` (the new default, nested inside `useNewTerrain`) — RETIRES the old independent-noise ocean (`getOceanFactor`'s `OCEAN_WARP`/`oceanNoise` stack) and lets **continentalness `C` author ocean placement AND depth in ONE heightmap**, the same "label and shape agree by construction" lesson PIPELINE-001 applied to biomes (this REVERSES PIPELINE-002's finding #5 "ocean cannot key off C"). Flag ON: `terrainSurface`'s BASE evaluates a dedicated `SPLINE_CONTINENTAL_OCEAN` (authored over C's REAL domain ~[−0.06,1.05], NOT the nominal [−1,1] — see agent-notes §4 dead-knots) at a relief-cliff-remapped, island-perturbed C — open ocean ~70 below sea, waterline (offset 0) at `COAST_THRESHOLD_C = 0.24`, land to +16; the fractal DETAIL (not the base) multiplies by a coast-band `SEAFLOOR_DETAIL` fade so open ocean reads as a smooth basin; `getOceanFactor` DISPATCHES to the injected `oceanFactorFromC` (identical 0..1 semantics, so the sand gate/`isTreeSoilSurface`/deltas/fjords/hydro spring-spill/every metric are unchanged consumers); `computePreRiverHeight` early-returns with NO legacy `oceanFloor` lerp (the spline already carved the seafloor); `floodSpill` widens its halo to `HYDRO_HALO_CONTINENTAL = 40` (deeper C basins). Inland seas at C-minima are allowed (supersedes OD5). **Flag OFF = byte-for-byte pre-CCR terrain (TGV 42)** — the escape hatch, proven identical over 14,400 cols ×3 seeds. All ~16 knobs live in the `'Continents & Oceans'` `GEN_TUNABLES` schema section (`ui:'both'`); the `CONTINENTAL_*` `continentalHeight`-shaping aliases apply UNCONDITIONALLY but are byte-identical at their frozen defaults. Gated by `tools/biome-pipeline-checks.mjs` (flag-branched calibration — flag-ON thresholds recalibrated in the CCR's Phase 3; `--legacy-ocean` keeps the pre-CCR calibration). **Per-world toggle (follow-up, build `2026-07-15.3`, NO TGV bump):** `continentalOceans` is now the 14th `DEFAULT_GEN_PARAMS` key (default true; `GEN_PARAM_SCHEMA` 'Terrain Shape' toggle row, 🌎 icon; `applyGenParams` sets `WORLD_CONFIG.continentalOceans = p.continentalOceans !== false`; `collectGenParamsFromUI`/`applyPreset` carry it) — the `flat`/`superflat` presets set it `false` so amplitude-0 flattens to dry land again (the spline-carved ocean base survives amplitude 0, which at `2026-07-15.2` regressed those two presets to ~12–33% flooded). default/amplified/archipelago/caves + all custom worlds keep C-oceans ON.
...
- **Transitive flow accumulation + flow-gated lowland widening (CCR-WORLDGEN-TECTONICS-008, build `2026-09-10.1`, `TERRAIN_GEN_VERSION` 45)** — `buildHydroRegion`'s flow map counted the kept paths CONTAINING a cell, but a tributary path ENDS at its join/capture cell, so the trunk downstream of a confluence never inherited its water: every trunk reach measured flow 1 (654/654 segs in a 70-spring/59-capture region) and `FLOW_WIDTH_SCALE` was inert. Now each path's unit of water also walks DOWNSTREAM along a first-claimant successor map (`nextOf`/`addDownstream`; reached-but-<3-cell stubs included via `stubEnds`), so confluences accumulate transitively. `riverFactorAt`'s `HYDRO_LOWLAND_WIDEN` is gated by flow through the new tunable `HYDRO_WIDEN_FLOW_LOG2` (default 3: gate 0.3 at flow 1 → 1.0 at flow 2³; 0 = the old flow-independent look) so a lone piedmont creek no longer carves an 84-block flooded estuary. The spring-keep hash now mixes the fractional seed's mantissa (`Math.floor(seed)` was 0 for every world). Flow is still WINDOWED per region build (a trunk only accumulates springs inside its own 1088-block build window) — unchanged design.
- **Valley-flank terrace break + piedmont lateral attraction (CCR-WORLDGEN-TECTONICS-009, build `2026-09-10.2`, `TERRAIN_GEN_VERSION` 46)** — owner screenshot: "ridges along slopes" = long parallel stepped contour bands on river-valley flanks. Root cause (measured): `applyRiverCarve`'s valley depression is a pure function of distance-to-channel, so its integer contour steps run parallel to the river (band-parallelism 0.27 vs 0.167 random). Fix (grep `CCR-WORLDGEN-VALLEY-TERRACE-001`): the `(gx,gz)` fed to the valley-band factor is coordinate-warped (`RIVER_VALLEY_WARP_AMP` 15 / `RIVER_VALLEY_WARP_FREQ` 0.006) and the contour wiggle multiplier is the tunable `RIVER_VALLEY_WIGGLE_MULT` (2.0, was a hardcoded 0.5) — parallelism 0.27 → 0.21 (seed 1337 down to the random baseline); tread AREA is unchanged by design. Piedmont "comb" (parallel hydro channels down a smooth mountain front, no confluences): `buildHydroRegion`'s descent step gained LATERAL ATTRACTION (`HYDRO_ATTRACT_RADIUS` 6 cells, `HYDRO_ATTRACT_SLOPE_MAX` 20 blk — on a near-planar step, re-rank the strictly-downhill neighbours by Euclidean distance to the nearest claimed cell; never steps uphill, so M15 holds) — confluence rate 0.83 → 0.87, coast-transect channel crossings −15–20%. `HYDRO_ATTRACT_RADIUS: 0` and `RIVER_VALLEY_WARP_AMP: 0` + `RIVER_VALLEY_WIGGLE_MULT: 0.5` reproduce the pre-009 carve.
...
- **Customization**: tree/cave density, terrain amplitude, sea level, biome size, noise persistence/lacunarity, spawn coords — ALL live on the default terrain path since CCR-WORLDGEN-UI-001 Phase D: amplitude is wired into `WORLD_CONFIG.terrainAmplitudeMultiplier` in `applyGenParams` (was dead — the missing "Step 11"); `seaLevel` reaches the worker via an injected `WORLD_DIMS.seaLevel` override baked at pool creation (the hand-maintained template literal stays 60 — see Lockstep Registry); spawn X/Z actually place the player + center pre-gen (`findAndSetSpawnPosition(spawnBX, spawnBZ)`); the dead `usePathBasedRivers` key was REMOVED (genParams is now 14 keys — `continentalOceans` added by the CCR-WORLDGEN-CONTINENTAL-OCEANS-001 follow-up; still "v3" — old saves carrying the removed key load fine, it's just never read). The knobs work through LIVE getters on `worldConfig`; the active world's params live in `activeWorldGenParams` (applied via `applyGenParams`, persisted as `savePacket.genParams`, restored BEFORE generation). In-session loads must `rebuildChunkWorkerPoolForActiveWorld()` (worker bakes config at pool creation). Details: agent-notes §3.
- **Preview**: real-time terrain preview (`WorldPreviewRenderer`) delegating directly to the game's own `blendedHeight()`/`getBiomeParams()` (no separate noise copy to keep in sync). With Tectonic Plates on it NEVER bakes erosion on the main thread (CCR-WORLDGEN-TECTONICS-009 `_orogenBakeSuppressed`; the label notes "erosion not previewed" when a region is uncached; preview-time hydro bakes are discarded by `applyGenParams`/`seedMainThreadNoise`'s `hydroField.clear()` before real generation). **Management**: rename, duplicate, import/export, storage stats, clear cache.
...
- **Rivers (hydrological, CCR-WORLDGEN-PIPELINE-002 WS6/Bump B)**: `hydroRivers`, `buildHydroRegion`, `floodSpill`, `hydroRegionOf`, `hydroLatticeH`, `hydroField` (RegionField, true-LRU — was `hydroRegionCache` before CCR-WORLDGEN-REGIONFIELD-001), `HYDRO_REGION`, `HYDRO_STEP`, `HYDRO_HALO`, `HYDRO_SPRING_H`, `FLOW_WIDTH_SCALE`, `nextOf`/`addDownstream`/`stubEnds` (transitive flow, CCR-WORLDGEN-TECTONICS-008), `HYDRO_WIDEN_FLOW_LOG2`
- **Region-bake tier (CCR-WORLDGEN-REGIONFIELD-001)**: `class RegionField` (single-sourced; worker built via `RegionField.toString()`; `putKey`/`hasKey` since CCR-WORLDGEN-TECTONICS-008), `orogenField` (clear-on-full), `hydroField` (true-LRU, delete-on-hit), `evictPolicy`, `sediment`/`climate` sockets (declared, empty). Orogen bake `buildOrogenRegion`→`tectonicErosionAt`; flow `tectonicRiverFactor`; talus `talusDh`→`tectonicTalusAt` (flag-ON, softens over-steep macro slopes on the `tectonicPlates===true && !forcedCentroid` path); recursion guard `_orogenBaking`. **CCR-WORLDGEN-TECTONICS-008:** the orogen region grid is ORIGIN-CENTERED (`floor(gx/OROGEN_REGION + 0.5)`; region (0,0) spans ±4096) so a default spawn bakes ONE region; the bake's height ordering is a stable radix sort (byte-identical to the old comparator sort, ~50× cheaper); bakes are SHARED — `preGenerateSpawnChunks` pre-bakes the spawn regions on main (`Baking tectonic erosion (n/N)...`) and `ChunkWorkerPool.syncOrogenToWorkers`/`shareOrogenRegion` broadcast them; a worker that self-bakes posts `{type:'orogenRegion'}` back (worker template `postNewOrogenBakes`/`_orogenPosted`), main caches + rebroadcasts (`_orogenShared`, reset at the orogen clear sites)
- **Tectonics plate frames (CCR-WORLDGEN-TECTONICS-008)**: `frameFor` (inside `plateLookup` — the ONE boundary-accumulation loop, evaluated per reference plate), `TECT_BLEND_BAND` (64, crest) / `TECT_BLEND_BAND_C` (256, deltaC/uplift/oceanicW) registry tunables (were internal consts until CCR-009), memo fields `oceanicW`, `rangeD2..rangeRegime2`, `rangeD3..rangeRegime3`, `rangeWS2/rangeWS3`. `plateLookup`'s `seedNum` mixes the fractional seed's mantissa (was `seed | 0` === 0 for every world — the plate layout was seed-invariant)
- **Tectonics shape polish (CCR-WORLDGEN-TECTONICS-009, flag-ON only)**: boundary domain warp is now FOUR octaves — grep `fMacro`/`fFine` in `plateLookup`; `BOUNDARY_WIGGLE_MACRO_FREQ_MULT` 0.30 / `BOUNDARY_WIGGLE_MACRO_AMP_MULT` 3.0 (belt-scale bow) and `BOUNDARY_WIGGLE_FINE_FREQ_MULT` 4.5 / `_FINE_AMP_MULT` 0.25 (sub-km coast detail); chosen by a straightness metric (RMS from a straight fit over 1500-blk windows, +30–80%) under a warp-Jacobian fold check (min det > 0.05 ×3 seeds), continuity, and a `plateLookup` cost cap (+4%; larger amplitudes blew the `_plateSiteCache` 8192-entry cap, not the fold bound). `orogenSampleRaster(gx, gz, reg, raster)` is the ONE injected warp + bilinear sampler shared by `tectonicErosionAt` / `tectonicTalusAt` / `tectonicRiverFactor` (they must sample the same warped coordinate); it adds a broad second warp octave (`OROGEN_WARP_BROAD_AMP_MULT` 4.0 × cell, `OROGEN_WARP_BROAD_FREQ` 0.0067) so ribbon-path belt rivers no longer read as 45°/90° D8 slots (grid-locked chords 87–94% → 22–70%). `_orogenBakeSuppressed` (declared in main, worker injection, and the harness stub) makes the three samplers return their neutral value for an uncached region while `WorldPreviewRenderer.render()` runs, so the create-world preview never bakes (label reads "erosion not previewed" when a region was skipped).
