...
- **`tools/biome-pipeline-checks.mjs`** (CCR-WORLDGEN-PIPELINE-001) — the M1–M14 biome-pipeline gate harness: field-histogram rail pile-up (M1), per-field autocorrelation length (M2), **biome↔shape agreement ★ (M3, ≥95%)**, label-boundary seam continuity ★ (M4), mountain coverage (M5), land/ocean split (M6), biome region-size (area-weighted, M7), river flood integrity (M8), no-grass-under-water / sand-water-proximity / no-alpine-invasion (M9/M10/M11, real Node material-cascade checks since Phase 3), determinism + feature determinism (M12/M13), river→ocean connectivity (M14, monitor-only). Runs against the REAL extracted functions (`classifyBiome`, `reliefParam`, `terrainSurface`/`blendedHeight` in the live flag mode) via `tools/lib/extract-terrain.mjs`'s `buildTerrainApi` (its `biomeDrivenTerrain` option tracks the live `WORLD_CONFIG.biomeDrivenTerrain` default unless explicitly overridden). `node tools/biome-pipeline-checks.mjs --seeds=1337,42,9001 [--json]`; human table default, exit 0 iff all GATING metrics pass all seeds.
