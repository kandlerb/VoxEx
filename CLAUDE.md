...
- **Tectonics plate frames (CCR-WORLDGEN-TECTONICS-008)**: `frameFor` (inside `plateLookup` — the ONE boundary-accumulation loop, evaluated per reference plate), `BLEND_BAND` (64, crest) / `BLEND_BAND_C` (256, deltaC/uplift/oceanicW) internal consts, memo fields `oceanicW`, `rangeD2..rangeRegime2`, `rangeD3..rangeRegime3`, `rangeWS2/rangeWS3`. `plateLookup`'s `seedNum` mixes the fractional seed's mantissa (was `seed | 0` === 0 for every world — the plate layout was seed-invariant)
