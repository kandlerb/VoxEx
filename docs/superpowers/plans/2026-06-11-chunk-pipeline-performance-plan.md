...
Scope: (a) port greedy meshing + corner-light packing + real shore/thickness generation into the worker mesher in `CHUNK_WORKER_CODE` — ALSO the `foamEdges` water attribute added 2026-06-11 (top-face land-adjacency bitmask; sync mesher computes it in `addFaceWaterIndexed`, `applyWorkerMeshData` zero-fills when worker data lacks it); (b) **fix the worker mesh VERTICAL OFFSET** — playtest 2026-06-11 showed worker meshes render ~20 blocks BELOW the true terrain (random pregen'd chunks; a sync remesh corrects them; `WORLD_DIMS.yOffset` is 0, so the bug is in the worker mesher's position math vs `renderChunk`'s — diff the Y handling between the two when starting this project); (c) complete `applyWorkerMeshData` per the missing list above (mirror `renderChunk`'s attach phase, ideally by EXTRACTING that attach phase into a shared function both paths call); (d) verify with a visual A/B (same seed, worker vs sync meshes must be pixel-identical) plus the existing worker round-trip test extended to compare face counts against the main-thread mesher. Only then flip `WORKER_MESH_PIPELINE_ENABLED` to true.

**Gate completeness note (2026-06-11):** the first gate only covered the two `processChunkQueue` dispatch sites; pre-generation reaches worker meshing via `renderChunkAsync` → `generateMeshViaWorker`, which became LIVE once the `voxelMaterial` ReferenceError was fixed (the throw had been masking it). The gate now also lives inside `generateMeshViaWorker` (authoritative) and `renderChunkAsync`'s early-out (skips the wasted round-trip). Any future caller of worker meshing is therefore gated automatically.
