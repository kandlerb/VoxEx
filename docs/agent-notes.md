...
| **`oceanFactor < 0.999` as a beach/shore proximity gate** (CCR-TERRAIN-011, fixed CCR-WORLDGEN-SCALE-001) | Never retry that threshold | `oceanFactor` fades gradually — ~0.93 at 40+ blocks inland, only >0.999 deep inland — so `<0.999` treats an ~80-block fade as "coast" -> wide inland sand APRONS / grass-sand camo across the sea-level Y-band (worse the more terrain sits at Y≈sea, e.g. after an R_BASELINE_CAP relief bump). Use a tight near-shore threshold: measured beach median of≈0.52, dry inland ≈1.0 -> shipped **`<0.60`** (thin tapering beaches; deep inland = grass). Change generateTerrainPass AND the isTreeSoilSurface mirror in lockstep (both injected). Do NOT shrink the Y-band instead — that regresses toward the banned pure-Y-band rule. |
| **dEdge-gated ocean-depth deepening** (distance-to-plate-boundary gate; CCR-WORLDGEN-SCALE-001b prototype) | Never retry | Premise "deep ocean = far from boundary (high dEdge)" is FALSE on waterline-hugging seeds: measured on 117658 dEdge ANTI-correlates with depth (shallow near-waterline platform sits at high dEdge; the trench sits at low dEdge near the boundary), so a dEdge gate deepens the wrong cols AND pushes the high-dEdge shelf under -> coast marches (-8.8pt land). The coast-preserving fix is a **C-GATE**: deepen by how far cc already sits below COAST_THRESHOLD_TECT (0 exactly at the coast contour) -> only lowers already-submerged cols, land fraction/coastline preserved by construction (0 ocean->land flips). Shipped SCALE-001b. |
| **`seed \| 0` / `Math.floor(seed)` as an integer seed** (found CCR-WORLDGEN-TECTONICS-008 in `plateLookup` AND `buildHydroRegion`'s spring-keep hash) | Never write again | `worldConfig.seed` is `SeededRandom`'s FIRST DRAW — a fraction in [0,1) — so both truncations were 0 for EVERY world: the whole plate layout (ids/jitter/oceanic/age/drift) and the spring-keep pattern were seed-invariant (276/289 sample points had the same `plateId` on two unrelated seeds; only the perm-table noise warp differed). Derive integer seeds from the mantissa: `Math.floor(seed * 4294967296) \| 0` (mixed through `plateHash32`/`Math.imul`), never from the integer part. Grep for `seed \| 0` / `Math.floor(seed)` before trusting any "seeded" hash. |
| **`TypedArray.sort(comparator)` inside a per-iteration sim loop** (fixed CCR-WORLDGEN-TECTONICS-008, `buildOrogenRegion`) | Never in a bake | A JS comparator sort of 262K indices cost 80 ms per call → 3.2 s of a 7 s bake at 40 iterations (75% of the sim phase). A stable LSD radix sort on a quantized key (3 passes × 11 bits, index tie-break via stability) is ~50× cheaper and was verified byte-identical over 2 regions (ties within 4.8e-7 blocks are the only theoretical difference; the LCG jitter makes them vanishingly rare). Same lesson as the string-key row: measure the "bookkeeping" around a sim, not just the physics. |
| **Summing boundary features only over the NEAREST plate's cell** (the original tect_core port; fixed CCR-WORLDGEN-TECTONICS-008) | Never re-introduce a nearest-only accumulation | A trench/arc/orogen band is 150–2200 blocks wide and geometrically crosses the Voronoi bisector into the neighbour's cell, but the neighbour's columns never summed it → the band ENDS at the edge → ruler-straight sea-floor cliffs and straight regime-2 coasts (measured C 0.020→−0.208 between adjacent columns). Any per-plate scalar (`deltaC`, `upliftR`, the `oceanic` flag) must be BLENDED across the frames of the nearest 2–3 plates near a flip line (`frameFor(ref)` + `(1−g/B)²` kernel, symmetric at g=0) — CCR-007's crest-only blend was the right idea applied to one term. A boolean per-plate gate on the height path (SCALE-001b's `s1.oceanic` deepen) is the same bug in miniature. |
| **Unscoped terrace-break warp on the default path** (measured CCR-WORLDGEN-TECTONICS-009 WS-A) | Never ship `TERRACE_WARP_AMP > 0` without an UPPER relief bound | `_twGate` rises from `TERRACE_WARP_RELIEF_MIN` and stays at full strength through R=1.0, so a flag-OFF default of 15–26 breaks the gentle-slope armor plates (render confirmed) but WORSENS `staircaseIndex` (M18-S, sampled on steep R≥0.6 fall-lines) by up to +21% on seed 9001. Scope the warp to the gentle band (see `TERRACE_WARP_RELIEF_MAX`) — steep terrain does not want it. |
| **Large plate-boundary warp amplitudes** (CCR-WORLDGEN-TECTONICS-009 WS-B sweep) | Cost bound, not fold bound, is what bites first | Macro-octave amplitudes ≥4× A scatter `plateLookup` queries across more lattice cells and blow the 8192-entry `_plateSiteCache` cap (+20–67% cost from repeated clears, same octave count). Shipped (0.30, 3.0) at +4%. If a stronger bow is ever wanted, raise the site-cache cap (or key blocks by a coarser cell) BEFORE raising amplitude. |
| **Counting paths-through-cell as "flow accumulation" when tributaries TERMINATE at joins** (fixed CCR-WORLDGEN-TECTONICS-008, `buildHydroRegion`) | Never | A tributary path ends at its join/capture cell, so the trunk downstream never inherits it — every trunk reach measured flow 1 and every flow-driven knob (`FLOW_WIDTH_SCALE`, confluence widening, deltas) was silently inert for the whole WS6 arc. Flow must walk DOWNSTREAM along a successor map after the join. Related lesson: renders that "look fine" at 4 blk/px hid this for weeks — check a per-segment flow histogram, not the picture. |
...
### Region-bake tier — `RegionField` (CCR-WORLDGEN-REGIONFIELD-001, build 2026-07-21.1)

Both per-region bake tiers run on ONE injected `class RegionField(cap, bake, evictPolicy)`. The pattern: a region key (`seed:rx:rz`) → a seeded, halo-bounded, deterministic `bake(rx,rz,seed)` → a cached multi-raster region object → a domain-warped bilinear **sampler** that reads it in the pure height chain. Byte-identity rests on ONE invariant: the cache is **pure memoization of a deterministic bake**, so output depends only on the bake body + key, NEVER on the cache/eviction machinery — a caching refactor cannot change output as long as the bake body and key stay character-for-character identical.

- **`orogenField`** (`evictPolicy` default `'clear'` = wipe-on-full, cap 12): bakes `buildOrogenRegion` → `{dh, flow, talusDh}`; sampled by `tectonicErosionAt` (Δh), `tectonicRiverFactor` (flow), `tectonicTalusAt` (talus). Recursion guard `_orogenBaking` is MODULE-scope and checked by the samplers BEFORE `.get()` (the bake samples `terrainSurface`, which calls the samplers — the guard makes that inner call return 0/1); RegionField does NOT own the guard.
- **`hydroField`** (`evictPolicy 'lru'` = delete-on-hit + evict-oldest, cap 64): bakes `buildHydroRegion` (now PURE — the old in-body self-memo dance moved into `RegionField.get`); sampled by `riverFactorAt`. True-LRU matters for the DEFAULT (hydroRivers-ON) path's hit-rate — see the §1 FIFO/LRU ledger row.
- Eviction policy is invisible to output (pure memo), only hit-rate — that is why both policies share one class. `sediment`/`climate` are declared-but-empty sockets for future rasters (CCR-2 lithology, rain-shadow climate).
...
### Tectonics speed + plate-frame blend + river flow (CCR-WORLDGEN-TECTONICS-008, build 2026-09-10.1)

- **Bake cost anatomy (512² region, EROSION_CELL 20, this machine):** sampling 3.4 s (262K `terrainSurface` calls, ~13 µs each flag-ON) + sim 4.3 s (D8 0.6, **comparator sort 3.2**, incision 0.2, talus 0.3) + post 0.1 → ~7.5 s. After 008: sort → radix (~0.1 s), `plateLookup` numeric keys + 5×5 site-block cache → **2.6–3.7 s**. `tools/scratch` timing recipe: copy voxEx.html, wrap the bake phases in `performance.now()`, build the api from the copy.
- **Bake sharing contract:** the bake is a pure function of (key, tunables) so ANY context's raster is authoritative. Main pre-bakes the spawn regions in `preGenerateSpawnChunks` Phase 1C (before the first generate job; postMessage order per worker guarantees arrival first) and `ChunkWorkerPool.generateTerrain` calls `syncOrogenToWorkers()` before every job (covers main-thread bakes from tree/spawn/preview paths). Workers post self-bakes back as `{type:'orogenRegion', key, reg}` — handled at the TOP of `_handleWorkerMessage` (no jobId; must not touch `workerBusy`). `_orogenShared` is per-pool and reset at both orogen clear sites. Null (beltless) regions are not shared (64 plateLookups to recompute).
- **Origin-centered orogen grid:** `rx = floor(gx/size + 0.5)`, `x0 = rx·size − size/2 − halo`. All three samplers + the bake + the pre-gen region set use the SAME formula — a drift here = wrong raster sampled = seams.
- **Frame blend:** `frameFor(ref)` is the single boundary loop; weights `w_k = (1−g_k/B)²` with `g_k = √d_k − √d_1`; frames 2/3 only evaluated when their weight is > 0 (~40% of columns at `BLEND_BAND_C` 256, ~10% at 64). s3/s4 identity swaps happen where `d3 = d4`, which needs a near-quadruple point to fall inside B — negligible for a jittered lattice. `dom*` (regime/conv/shear) stay pure-s1 diagnostics. Measured: `plateLookup` 1.6 → 2.5 µs/col chunk-coherent for the blend; spawn-window adjacent steps > 30 blk 7 → 0.
- **Still straight at map scale:** belts/coasts follow the bisectors; `BOUNDARY_WIGGLE_*` scale with `PLATE_SIZE/12000` → ~2000-blk wavelength at PLATE_SIZE 5000. Not addressed here.
- **`terrain-probe.mjs` trap (fixed):** its path flags passed a hard `false` when omitted, so every bare render since the biome-driven/hydro flips was of a non-shipping world. The straight D8 slot "rivers" seen in such renders are `tectonicRiverFactor` (ribbon path only). Always render with the flags you mean, and `--game-seed` to match an in-game seed.

### Slope terraces, bent boundaries, organic belt rivers, no-bake preview (CCR-WORLDGEN-TECTONICS-009, build 2026-09-10.2)

Five Sonnet workstreams in isolated worktrees, merged by the integrator (conflicts only in the shared `GEN_TUNABLE_SCHEMA` 'Rivers' block — keep both sides). Each shipped with a metric, not an eyeball:
- **Valley terraces**: `bandParallelism` (fraction of integer step edges parallel to `grad(vfRaw)`) is the metric that moved; `wideTerrace` did not — a coordinate/value warp changes band DIRECTION, not tread AREA. Harness recipe: `tools/scratch/valley-metrics.mjs` in the WS-A worktree (real extracted `blendedHeight`/`riverFactorAt`).
- **Boundary straightness**: RMS perpendicular deviation from a straight fit over 1500-blk windows of `dEdge < 24` points (`tools/scratch/tect-straightness.mjs`); "ruler-straight" = RMS < 15. Fold safety = min Jacobian det of the warp map over 90k samples.
- **Belt rivers**: grid-locked chord fraction (±6° of 0/45/90/135) + mean turn angle on the `tectonicRiverFactor < 0.6` skeleton. All three orogen samplers MUST share `orogenSampleRaster`'s warp or the river leaves its carved valley.
- **Lateral attraction** re-ranks only already-downhill neighbours (Euclidean, not Chebyshev — Chebyshev is blind to lateral nudges when the along-flow axis dominates). Raising `HYDRO_CAPTURE_RADIUS` instead makes one artificial long jump and discards more stubs.
- **Preview no-bake**: the samplers check `orogenField.hasKey` under `_orogenBakeSuppressed`; real generation is byte-identical (flag-ON grid hash unchanged). The flag lives in three scopes (main / worker injection / harness stub) — add a fourth declaration if a new scope ever assembles the samplers.
...
- **Terrain scale (CCR-WORLDGEN-SCALE-001, TGV 44, build 2026-07-21.3):** owner "meet in the middle" — keep biomes/expanses large but not insurmountably flat. Shipped `R_BASELINE_CAP` 0.55 (interior relief; sanctioned interior-hills use — NOT the mountain-dial crank do-not-retry), `PLATE_SIZE` 5000, `OROGEN_WIDTH` 3000, `HYDRO_CHANNEL_HALF_WIDTH` 12 (wider rivers), shoreline sand `oceanFactor<0.60`. Owner approved slopes via fly-test. **flag-OFF fingerprint baseline is now `cf5bc1df…f162114` (TGV 46, CCR-WORLDGEN-TECTONICS-009 — gentle-slope terrace warp ON by default, valley-band warp, lateral attraction)**; `e02bfb2a…242a7e` = TGV 45; `cd1df4af…b14d7` = TGV 44; `7487c195…b086b3e46` = TGV 43. TGV bumps + world regen are FINE (solo player) — see memory [[voxex-performance-scalability]].
...
- **Gentle-slope terrace warp is ON for BOTH paths since CCR-WORLDGEN-TECTONICS-009** (`TERRACE_WARP_AMP` 20, band-scoped by `TERRACE_WARP_RELIEF_MAX` 0.55 so steep terrain is untouched; flag-ON total stays 26 via `TERRACE_WARP_AMP_TECT` 6). Integrator decision against the M18-S staircaseIndex proxy (+3–5% on steep fall-lines at the 0.55 ceiling): the owner's screenshot complaint about parallel contour bands on gentle slopes wins; `TERRACE_WARP_AMP: 0` restores the old look.
- **Terrace armor-plates on GENTLE slopes (CCR-WORLDGEN-SCALE-001c, build 2026-07-21.5):** the TERRACE_WARP contour-break domain warp is RELIEF-GATED (TERRACE_WARP_RELIEF_MIN), so it only fired on STEEP terrain -- the gentle interior slopes from SCALE-001 (R_BASELINE_CAP 0.55) sat below the gate and terraced into regular parallel diagonal bands (owner: armor plates). Verified isotropic contour-stepping, NOT a noise grain. Fix (flag-ON): TERRACE_WARP_RELIEF_MIN 0.4->0.15, TERRACE_WARP_AMP_TECT 6->26, TERRACE_WARP_FREQ 0.02->0.009 (broad meanders; owner higher-amp-less-swirly). COORDINATE warp not added surface noise, so the clean-steps preference above is preserved (steps made irregular, not textured). A strong warp CAN fold (amp*2pi*freq>1 -> adjacent-col cliffs) but this setting does not -- the maxAdjStep-40 scarp near (1146,1141) seed 117658 is a REAL tectonic scarp (identical at amp 6 and 26; warp adds no new continuity spikes).
