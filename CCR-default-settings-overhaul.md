...
**Date:** 2026-06-21
**Status:** ✅ **IMPLEMENTED 2026-06-21** in `voxEx.html` (build `2026-06-21.2`, `SETTINGS_VERSION = 2`). All decisions D-1…D-4 resolved (Part D). Static verification done via Read/Grep (profiles confirmed untouched, no duplicate identifiers, all sentinel reads safe). **Still needs in-browser test** — see Part E checklist (worker-count log on 4/8/14-core, memory budget shows "Auto (N)", one-time settings revert on an old save, and `tools/voxex-tests.html`).
